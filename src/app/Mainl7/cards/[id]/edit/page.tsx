...
export default function EditCardPage() {
  ...
  const { data: card } = trpc.cards.getById.useQuery({ id: cardId });
  ...
  useEffect(() => {
    if (card && !loaded) {
      // drizzle join returns { cards: {...}, occasions: {...} }
      const c = (card as any).cards ?? card;
      setForm({
        titleAr: c.titleAr,
        title: c.title,
        imageUrl: c.imageUrl,
        ...
        defaultPositionX: c.defaultPositionX,
        defaultPositionY: c.defaultPositionY,
        defaultFontFamily: c.defaultFontFamily,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    await updateCard.mutateAsync({ id: cardId, ...form });
  };

  const cardData = card ? ((card as any).cards ?? card) : null;
  if (!cardData) return (
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-black text-gray-800">تعديل البطاقة</h1>
          <p className="text-gray-500 text-sm">{cardData.titleAr}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={updateCard.isPending || uploading}
